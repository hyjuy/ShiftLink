...
작성: 2026-09-20. 이 문서는 `docs/mock-mes-claude-upgrade-template.md`의 실행 결과로, 병렬 개발의 기준 계약이다.
...
- 기준정보: `docs/00_plant_and_relations.json` (설비 10대 EQ-0001~0010, material_flow 4건 — EQ-0006→0007→0008→{0009(60 m_min), 0010(20 m_min)} 분기).
...
- `from_catalog(catalog_data: dict) -> Configuration` — 기존 `docs/00_plant_and_relations.json`에서 기본 구성 A 생성. 원본 파일은 불변. 주경로는 material_flow 위상 + **현행 동작 보존을 위해 문서화된 규칙**으로 한 번만 산출하고 Configuration에 명시 고정.
