...
| Package migration GREEN | `uv run --with pytest --with pydantic python -m pytest tests/test_schemas.py -q` | `1 passed` |
| Coverage | `uv run --with pytest --with pydantic --with pytest-cov python -m pytest tests/test_schemas.py -q --cov=shiftlink.agent.schemas --cov-report=term-missing` | `69 statements, 100%` |
