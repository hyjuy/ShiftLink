...
`/api/state`는 활성 `config_id`를 포함한다. 구성 적용은 안전한 런 경계에서만 수행되며(가동 중 hot swap 없음), 성공 시 이전 런을 보존하고 새 구성의 새 런을 만든다. 되돌리기는 이전 구성을 참조하는 새 런이다. 상세는 `docs/design/mock-mes-modularization.md` 참고.
