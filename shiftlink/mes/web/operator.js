...
(() => {
  ...
  function layout(model) {
    ...
    for (const group of groups) {
      ...
      group.forEach((n,i)=>placed.push({...n,x:(i%cols+0.5)*960/cols,y:62+(row+Math.floor(i/cols))*154}));
      row+=Math.ceil(group.length/4);
    }
    return {nodes:placed,height:Math.max(180,row*154+8)};
  }
  ...
  function flowView(model, experience="learn", filter="related", selectedRelation="") {
    ...
    const edges=links.map((l,i)=>{
      ...
      const lateral=a.y===b.y, sx=a.x+(lateral ? Math.sign(b.x-a.x)*91 : 0), sy=a.y+(lateral ? 0 : Math.sign(b.y-a.y)*48), ex=b.x-(lateral ? Math.sign(b.x-a.x)*96 : 0), ey=b.y-(lateral ? 0 : Math.sign(b.y-a.y)*52);
    ...
    const nodes=map.nodes.map(n=>{
      ...
      const dots=n.coils.map((c,i)=>`<circle class="flow-coil ${c.quality_status==="hold"?"held":""}" cx="${20+Math.max(0,Math.min(1,Number(c.position)||0))*60}" cy="-26" r="7"><title>${esc(c.coil_id)} · ${Math.round(Number(c.position)*100)}%${c.quality_status==="hold"?" · 제품 보류":""}</title></circle>`).join("");
      return `<g transform="translate(${n.x} ${n.y})" class="flow-node ${cls} ${activeId===n.equipment_id?"selected":""}" role="button" tabindex="0" data-equipment="${esc(n.equipment_id)}" aria-pressed="${activeId===n.equipment_id}" aria-label="${esc(description)}"><title>${esc(description)}</title><rect class="node-body" x="-91" y="-46" width="182" height="96" rx="10"/><text class="node-code" x="-78" y="-23">${esc(title(n))}</text><text class="node-status" x="-78" y="-3">${fault?"! ":held?"Ⅱ ":waiting?"↳ ":""}${esc(state)}</text><text class="node-role" x="-78" y="17">${esc(n.role[0])}</text><text class="node-meta" x="-78" y="37">${esc(component || group)}</text>${dots}</g>`;
    }).join("");
