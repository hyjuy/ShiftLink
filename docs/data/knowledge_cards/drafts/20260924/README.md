...
이번 승인은 보관 위치 승인이다. K-0301~0303의 원문·내용 검수는 사용자가 완료했다고 확인했다. 냉간코일 공정 적용성·현장 안전 검토·출처 이용 범위·운영 KB 채택은 별도 미확인이다. `accepted/L1`로 변경하지 않았다.

런타임 검색은 `accepted`·`kb`·`L1`을 모두 만족하는 카드만 적재하므로 이 초안 3장은 검색되지 않는다. 승격 전에는 냉간코일 공정 적용성, 현장 안전, 출처 이용 범위, P5 규칙·judge 게이트와 운영 KB 채택을 확인하고 결과를 기록해야 한다. 운영 승인 없이 상태 필드만 바꿔 통합 시험에 사용하지 않는다. 모델 연동 시험에는 `eval/fixtures/dev/category_f_e2e.json`의 합성 카드를 사용한다.
