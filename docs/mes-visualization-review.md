...
- `docs/mock-mes-domain.md`: 스크랩 경로는 시연에서 제외되지만 화면에서는 다른 지원 설비와 함께 표시된다. 실제 이송 분기와 유틸리티는 의미를 나누어 표현할 필요가 있다.
- `docs/mock-mes-recovery.md`: 부품 건전도와 정상 확인은 합성 모델이다. 모의값 명시는 장점이나 실제 잔여수명 판단 근거로 사용할 수 없다.
